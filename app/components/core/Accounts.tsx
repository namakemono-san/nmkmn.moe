        ...
        <div className="flex-col">
            {accounts.map((account) => (
                    <a
                        ...
                        key={account.id}
                    >
                        <div className="backdrop-blur-[2px] bg-theme bg-opacity-[2%] hover:bg-opacity-10 rounded-xl flex overflow-hidden">
                            <div className="text-white w-1/5 p-2 px-4 font-semibold" style={{background: account.color}}>{account.name}</div>
                            <div className="p-2 flex flex-col">
                                <div>
                                    {account.label.split("|").map((label, i) => (
                                        <span key={label} className="underline text-theme">{label}</span>
                                    ))}
                                </div>
                            </div>
                        </div>
